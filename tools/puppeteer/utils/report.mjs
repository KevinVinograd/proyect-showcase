 ...
 * Groups by viewport, sorts by severity, deduplicates info-level noise.
 */
export function printReport(results, url) {
  ...
  console.log(`  Viewports: ${byViewport.size}  |  Scenarios: ${new Set(results.map(r => r.scenario)).size}`);
  console.log(c('dim', `  ${'\u2500'.repeat(54)}`));
